...
3. **"重复率 73.86% 是不是质量很差？"**
   这要看口径。我们现在用**入队口径**（7 天窗口内新抓进来的候选里，重复变体占比）衡量 G4——它反映"抓取进来的噪音"，不受历史堆积影响。当前高是因为 Pilot 才开始两天，窗口内几乎都是 Day-1 批量抓取 + 已筛内容又被重复抓到的变体。**去重引擎已经正确识别重复**：队列默认折叠成簇代表（210 条 → 53 行），标着 `already SRC-…` 的是内容已入库、建议直接驳回的。剩下的人工 triage 会把这些重复清掉，后续配合入站跳过（RCP 候选）从源头降噪。

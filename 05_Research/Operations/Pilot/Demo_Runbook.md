...
3. **"重复率 73.86% 是不是质量很差？"**
   不完全是——这个数反映的是当前临时库的状态：每天自动抓取会和已筛过的来源重复，而**已驳回的候选会被 30 天保留策略清理**（审计记录仍在），剩在队列里的大多是重复变体。这正是我们 G4 去重优化（B-014 后续）要解决的工程项。
...
- **Gate**：G1 20/20 ✅ · G6 96/20 ✅ · G7 96/96 ✅ · **G4 重复率 73.86%（<15%）⬜**
- **当前库**：306 候选（210 待筛 + 96 已提升）；驳回候选按 30 天保留策略清理，239 条驳回审计保留

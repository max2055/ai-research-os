...
class ScheduleTests(unittest.TestCase):
    ...
    def test_every_n_units(self) -> None:
        self.assertEqual(timedelta(hours=6), interval_from_schedule("every 6 hours"))
        self.assertEqual(
            timedelta(minutes=30), interval_from_schedule("every 30 minutes")
        )
        self.assertEqual(timedelta(days=2), interval_from_schedule("every 2 days"))
        self.assertEqual(timedelta(weeks=1), interval_from_schedule("every 1 week"))

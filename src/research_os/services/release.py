...
from research_os.domain.policies import FORBIDDEN_INVESTMENT_ACTIONS
...
def _verification_covers_tree(root: Path, record: str) -> bool:
    commit = _line_value(record, "Commit").strip("`*")
...
def _impact_gate_evidence(
    ...
    human_approved = _current_human_evidence(
        reviewer, approved_date, as_of
    ) and decision.lower().startswith("approve")
...
def release_readiness_v03(
    ...
    *,
    ...
    mode_passed, mode_observed, mode_payload = _mode_gate_evidence(root, as_of_date)
    ...
    forbidden_recommendations = [
        obj.object_id
        for obj in recommendations
        if FORBIDDEN_INVESTMENT_ACTIONS.search(
            obj.body
            + "\n"
            + "\n".join(
                str(value)
    ...
    checks += [
        ...
        _check_v03(
            ...
            + ", ".join(forbidden_recommendations),
            LIMITATIONS_PATH,
    ...
    recovery_passed = _dated_pass_record(recovery, as_of_date)
    quality_rows = all(
        bool(
            re.search(
    ...
    performance_date = _record_date(performance)
    performance_rows = re.findall(
        r"(?mi)^\|\s*(?:Home|Candidate Queue|Company|Sector|Impact 3-hop|"
        r"Full validate|Index render)\s*\|[^\n]*\|\s*pass\s*\|\s*$",
        performance,
    )
    candidate_p95_match = re.search(
        r"(?mi)^\|\s*Nearest-rank p95\s*\|\s*([0-9]+(?:\.[0-9]+)?)\s*s\s*\|",
        performance,
    )
    performance_passed = (
        _display_status(performance) == "passed"
        and performance_date is not None
        and performance_date <= as_of_date
        and len(performance_rows) == 7
        and candidate_p95_match is not None
        and float(candidate_p95_match.group(1)) < 2.0
        and "authoritative writes: 0" in performance
    )

    migration = _read(root, MIGRATION_PATH)
    migration_date = _record_date(migration)
    migration_passed = (
        _display_status(migration) == "passed"
        and migration_date is not None
        and migration_date <= as_of_date
        and "rollback precondition changed" in migration
        and "Candidate DB schema version: 2" in migration
        and "0 errors, 0 warnings" in migration
    )
    ...
    recovery_boundaries_passed = recovery_passed and all(
        term.lower() in recovery.lower() for term in boundary_terms
    )
    security_test = _read(root, "09_Automation/tests/test_m6_security.py")
    dashboard_security_passed = (
        recovery_passed
        and bool(re.search(r"(?mi)^\|\s*Dashboard\s*\|[^\n]*\|\s*pass\s*\|", recovery))
        and "test_dashboard_mutation_routes_remain_allowlisted" in security_test
        and "test_traversal_payloads_never_escape_repository" in security_test
        and "test_secrets_never_appear_in_health_html" in security_test
    )
    checks += [
        _check_v03(
            "engineering.quality_suite",
        ...
        _check_v03(
            ...
            (
                ...
                f"{candidate_p95_match.group(1)}s; authoritative writes 0"
                if performance_passed and candidate_p95_match is not None

...
<a class="button-link" href="/operations/actions/new">新建 Action</a>
<a class="button-link" href="/operations/discovery">查看通道</a>
<a class="button-link" href="/operations/backups">备份</a></div></section>
<section class="metrics"><div class="metric"><strong><a class="hero-link" href="#operations-queues">{esc(len(snapshot["schedules"]) + len(snapshot["actions"]) + len(snapshot["reviews"]) + len(snapshot["forecasts"]) + len(snapshot["recommendations"]))}</a></strong><span>待处理</span>
<p class="muted">截至 {esc(snapshot["as_of"])}</p></div></section>
<section class="panel" id="operations-queues"><h3>调度</h3>{table(["Schedule", "Job", "Target", "Next run", "状态"], schedule_rows)}</section>
...
def create_app(
    ...
    async def lifespan(_: FastAPI) -> AsyncIterator[None]:
        ...
        try:
            ...
            supervisor.stop()
            review_ai_executor.shutdown(wait=False, cancel_futures=True)
